...
Graph::Graph(AdjMap adj) {
    this->adj = new AdjMap(std::move(adj)); // heap allocation better for large map. make this smart ptr?
...
std::tuple<Graph::TreeDecompAdj, Graph::TreeDecompBags, unsigned long> Graph::get_td() {
    ...
    //     for (const auto& neighbor : neighbors) {
    //         // might have to do this at each step with h, as adj does not have proper fill-in edges necessary to make bag weights
    //         if (v == neighbor) {td_weights[v].push_back(0);}
    //         else td_weights[v].push_back(get_edge_weight(v, neighbor));
    //     }
    // }
...
std::vector<unsigned long> Graph::get_bag_path(const unsigned long v) const {
    ...
    if (!td_bags.contains(v)) {
        return path; // Return empty path if bag doesn't exist
    }

    // If v is the root, return just the root
    if (v == td_root) {
        path.push_back(td_root);
        return path;
    }

    // DFS using stack to find path from td_root to v
    std::unordered_map<unsigned long, unsigned long> parent;
    std::vector<unsigned long> stack;
    std::unordered_set<unsigned long> visited;

    stack.push_back(td_root);
    visited.insert(td_root);
    parent[td_root] = td_root;

    bool found = false;
    while (!stack.empty() && !found) {
        unsigned long current = stack.back();
        stack.pop_back();
        ...
        if (td_adj.contains(current)) {
            for (unsigned long neighbor : td_adj.at(current)) {
                if (!visited.contains(neighbor)) {
                    visited.insert(neighbor);
    ...
    // Reconstruct path from td_root to v
    if (found) {
        ...
        for (unsigned long i = 0; i < anc.size()-1; i++) {
            ...
            for (unsigned long j = 0; j < bag.size()-1; j++) {
                ...
                if (pos[v_bag][j] > i) {
                    ...
                    unsigned long dis_index = pos[v_bag][j];
